...
var path =
  window.location.hostname + ":" + window.location.port + appName + "websocket";
...
export default function WebRTCAdaptor(initialValues) {
  ...
  this.setDesktopwithCameraSource = function (
    stream,
    streamId,
    audioStream,
    onEndedCallback
  ...
  this.prepareStreamTracks = function (
    mediaConstraints,
    audioConstraint,
    stream,
    streamId
  ...
  this.leave = function (streamId) {
    var jsCmd = {
      command: "leave",
      streamId:
        thiz.isMultiPeer && thiz.multiPeerStreamId != null
          ? thiz.multiPeerStreamId
          : streamId,
    };
   ...
   *
   */
  this.setVideoCameraSource = function (
    streamId,
    mediaConstraints,
    onEndedCallback,
    stopDesktop
  ...
  this.updateVideoTrack = function (
    stream,
    streamId,
    mediaConstraints,
    onEndedCallback,
  ...
  this.initPeerConnection = function (streamId, dataChannelMode) {
    if (thiz.remotePeerConnection[streamId] == null) {
      ...
      if (dataChannelMode == "publish") {
        ...
        if (thiz.remotePeerConnection[streamId].createDataChannel) {
          var dataChannelPeer = thiz.remotePeerConnection[
            streamId
          ].createDataChannel(streamId, dataChannelOptions);
